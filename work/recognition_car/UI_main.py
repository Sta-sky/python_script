...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        # setupUi方法为创建UI界面  调用的都是PyQt中的方法  不用纠结
    ...
    def retranslateUi(self, MainWindow):
        # 对setupUi方法中创建出来的组件  进行文字填充
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "毕设车牌识别系统"))
    ...
    def __vlpr(self, path):
        ...
        # 加载识别方法对象 PlateRecognition
        PR = PlateRecognition()
        # 传入图片路径并调用识别接口  返回识别记过
        result = PR.VLPR(path)
        return result

    def __show(self, result, FileName):
        # 显示表格
        # self.tableWidget 是表格
