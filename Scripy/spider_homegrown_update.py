...
class SpiderVideo(object):
    def __init__(self):
        ...
        self.new_save_path = 'F:\\IMT\\kele\\{}.mp4'
...
class MySpiderThread(Thread, SpiderVideo):
    ...
    def run(self):
        ...
        while True:
            ...
            request_res = self.get_m3u8_save_video(link, self.name, self.alive_staatus)
            if not request_res:
                continue
            else:
                key, ts_urls, new_save_path, file_name = request_res
                self.decrypt_save_ts(key, ts_urls, new_save_path, file_name,
                                     self.name)
            self.lock.acquire()
...
if __name__ == '__main__':
    ...
    for num in range(5):
        th_name = '党_的线程_' + str(num) 
        my_th = MySpiderThread(th_name)
