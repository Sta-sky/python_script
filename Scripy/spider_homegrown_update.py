...
class Spider_Video():
	...
	def get_video_id(self):
		...
		try:
			for page in range(self.start_page, self.end_page):
				html_data = self.return_requests_data(self.url.format(page)).text
	...
	def get_m3u8_save_video(self, link_list):
		totle = len(link_list)
		for link in link_list:
			self.video_num += 1
			print(f'当前视频的链接地址为{link}')
			page_data = self.return_requests_data(link).text
			m3u8_re = '(.*)\.m3u8'
			...
			print(m3u8_file_res_url)
			ele = etree.HTML(page_data)
			try:
				self.name = ele.xpath('//title/text()')[0]
			except Exception as e:
				self.name = random.randint(1, 100)
				print(f'没有匹配出name，当前的name为随机数 ： {self.name}')
			...
			print(f'===总共{totle}个影片，正在下载第{self.video_num}个====================')
...
if __name__ == '__main__':
	video = Spider_Video()
	hrefs_list = video.get_video_id()
	video.get_m3u8_save_video(hrefs_list)
